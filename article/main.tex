...
are different things, we intuit that writing style is defined in terms of both.
...
Opinions that style depends on context and author choice vis-a-vis a
communication goal seem to converge~\cite{mcdonald1985computational,
    hovy1987generating}.
...
Our approach stands out for its simplicity and focus.
...
They categorize HLFs by domain and family and make available information on the
...
We notice these results are similar to those in the first experiment scenario.
...
This suggests that LLMs are particularly adept at following instructions
regarding HLFs when those instructions are more concrete.
...
LLM under the influence of each individual HLF is another limitation of our
approach.
...
To our surprise, it turns out that instructing large language models to write
text that conforms to a selected assortment of linguistic features leads to
significantly different linguistic featues of the generated text.
This is especially true for concrete instructions.

However, the outcomes are not always desirable.
