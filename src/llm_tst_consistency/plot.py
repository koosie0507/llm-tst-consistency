...
def _add_variance(
    ...
    for feature in features:
        df[f"{name_extractor(feature)}_varlo"] = df[f"{name_extractor(feature)}"] - benchmark_stats[feature].variance
        df[f"{name_extractor(feature)}_varhi"] = df[f"{name_extractor(feature)}"] + benchmark_stats[feature].variance
    return df
...
def _create_benchmark_trace(
    df: pd.DataFrame, feature: str, title: str, benchmark_value: float, color: str = "#575757", dash: str = "dot"
) -> BaseTraceType:
    return go.Scatter(
        x=df.index,
...
def _add_trace_to_plot(fig: Figure, trace: BaseTraceType, i: int, col_count: int) -> None:
...
def draw_plots(
    ...
    fig = make_subplots(rows=len(features) // col_count + 1, subplot_titles=features, cols=col_count)

    for i, f in enumerate(features):
        for trace in [
            _create_hlf(df, f),
            _create_baseline(df, f),
            _create_benchmark_trace(df, f, f"{f} benchmark min", benchmark_stats[f].low),
            _create_benchmark_trace(df, f, f"{f} benchmark max", benchmark_stats[f].high),
            _create_benchmark_trace(df, f, f"{f} benchmark average", benchmark_stats[f].mean, "#000000", "solid"),
        ]:
